...
export default async function Deductions(req: NextApiRequest, res: NextApiResponse) {
    if (req.method == 'POST') {
        const { type, description, frequency } = req.body
        const token = await database.getConnectionToken()
        const apiUrl = (await database.isSandbox()) ? sandboxApiUrl : finchApiUrl

        const axiosRes = await axios.post(`${apiUrl}/employer/benefits`, {
            type: type,
        ...
        const axiosRes = await axios.get(`${apiUrl}/employer/benefits`, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Finch-API-Version': '2020-09-17',
                'Content-Type': 'application/json',
